...
if [ "$(uname)" != "Darwin" ]; then
  m "错误: lidkeep 只能在 macOS 上构建运行。" \
    "Error: lidkeep can only be built and run on macOS." >&2
  exit 1
fi
...
if [ "$1" = "--cli-only" ]; then
  ...
  sudo make install-cli 2>/dev/null || {
    ...
    install -m 0755 build/lidkeep "$HOME/.local/bin/lidkeep"
    m "已安装到 ~/.local/bin/lidkeep（确保该目录在 PATH 中）" \
      "Installed to ~/.local/bin/lidkeep (make sure it is on your PATH)"
  }
else
  make install
fi
